-- Event discovery + social features schema extension

-- Extend events metadata for richer discovery cards and admin management
...
-- Keep `image` and `image_url` synchronized for backward compatibility
UPDATE public.events
SET image = COALESCE(image, image_url)
WHERE image IS NULL;

-- Public users profile table requested by product spec
...
-- Backfill users table from auth + profiles when available
INSERT INTO public.users (id, email, name, created_at)
SELECT au.id, COALESCE(au.email, 'unknown@example.com'), au.raw_user_meta_data ->> 'full_name', now()
FROM auth.users au
ON CONFLICT (id) DO NOTHING;

UPDATE public.users u
SET
  name = COALESCE(u.name, p.full_name),
  email = COALESCE(NULLIF(u.email, ''), p.email, u.email)
